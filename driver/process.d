/**
 * Presto ODBC Driver Edits:
 *
 * Around line 520 we commented out a few lines that do not function correctly
 * when you are running from a DLL instead of a normal executable. Namely,
 * binding stdin, stdout, stderr does not work correctly.
 *
 * It is possible that this will be fixed in a future version of Phobos,
 * in particular, we noted that the version on Github looks like it no longer
 * calls the function we identified as being problematic (though it may be
 * broken for other reasons). For now, we chose to simply perform these
 * edits.
 *
 */
    ...
    @trusted
{
    ...
    //Presto ODBC Driver: Commented out the following lines because they result in an
    //excpetion being thrown when you call this function from a DLL as opposed to an
    //executable.
...
version (Windows) version(none) unittest
{
...
version(none) unittest // Command line arguments in spawnProcess().
{
...
version(none) unittest // Environment variables in spawnProcess().
{
    // We really should use set /a on Windows, but Wine doesn't support it.
    version (Windows) TestScript envProg =
       `if [%STD_PROCESS_VERSION(NONE) UNITTEST1%] == [1] (
            if [%STD_PROCESS_VERSION(NONE) UNITTEST2%] == [2] (exit 3)
            exit 1
        )
        if [%STD_PROCESS_VERSION(NONE) UNITTEST1%] == [4] (
            if [%STD_PROCESS_VERSION(NONE) UNITTEST2%] == [2] (exit 6)
            exit 4
        )
        if [%STD_PROCESS_VERSION(NONE) UNITTEST2%] == [2] (exit 2)
        exit 0`;
    version (Posix) TestScript envProg =
       `if test "$std_process_version(none) unittest1" = ""; then
            std_process_version(none) unittest1=0
        fi
        if test "$std_process_version(none) unittest2" = ""; then
            std_process_version(none) unittest2=0
        fi
        exit $(($std_process_version(none) unittest1+$std_process_version(none) unittest2))`;

    environment.remove("std_process_version(none) unittest1"); // Just in case.
    environment.remove("std_process_version(none) unittest2");
    assert (wait(spawnProcess(envProg.path)) == 0);
    assert (wait(spawnProcess(envProg.path, null, Config.newEnv)) == 0);

    environment["std_process_version(none) unittest1"] = "1";
    assert (wait(spawnProcess(envProg.path)) == 1);
    assert (wait(spawnProcess(envProg.path, null, Config.newEnv)) == 0);

    auto env = ["std_process_version(none) unittest2" : "2"];
    assert (wait(spawnProcess(envProg.path, env)) == 3);
    assert (wait(spawnProcess(envProg.path, env, Config.newEnv)) == 2);

    env["std_process_version(none) unittest1"] = "4";
    assert (wait(spawnProcess(envProg.path, env)) == 6);
    assert (wait(spawnProcess(envProg.path, env, Config.newEnv)) == 6);

    environment.remove("std_process_version(none) unittest1");
    assert (wait(spawnProcess(envProg.path, env)) == 6);
    assert (wait(spawnProcess(envProg.path, env, Config.newEnv)) == 6);
}

version(none) unittest // Stream redirection in spawnProcess().
{
    version (Windows) TestScript prog =
       "set /p INPUT=
        echo %INPUT% output %~1
...
version(none) unittest // Error handling in spawnProcess()
{
...
version(none) unittest // Pid and wait()
{
...
// version(none) unittest: This function is tested together with kill() below.
...
version(none) unittest // tryWait() and kill()
{
    import core.thread;
...
// Version(None) Unittest support code:  TestScript takes a string that contains a
...
version(none) unittest
{
    ...
    // in addition to the version(none) unittest_burnin one below which requires
    // special preparation.
...
{
    // The version(none) unittest for this function requires special
...
//version = version(none) unittest_burnin;
...
version(none) unittest
{
    ...
    // To run this unit test, create std_process_version(none) unittest_helper.d with the
    ...
    // rdmd --main -version(none) unittest -version=version(none) unittest_burnin process.d

    auto helper = absolutePath("std_process_version(none) unittest_helper");
